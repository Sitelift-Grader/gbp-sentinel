"""Export consolidated master escalation dossier and forum threads."""
...
def export_master_dossier():
    ...
    # Get all targets, submissions, verification status, and location counts
    cur.execute("""
        SELECT t.name, t.website, t.hq_address, s.case_id,
               COALESCE(s.google_status, 'pending_email_verification') AS google_status,
               s.email_verified, COUNT(l.id) AS loc_count
        FROM targets t
        JOIN submissions s ON t.name = s.target_name
        LEFT JOIN locations l ON t.name = l.target_name
        GROUP BY t.name, s.case_id, s.google_status, s.email_verified
        ORDER BY t.name ASC
    """)
    rows = cur.fetchall()
    ...
    # Build Master Markdown report for Sterling Sky & Google Trust & Safety (English)
    report_en = []
    ...
    report_en.append("")
    report_en.append(f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report_en.append(f"**Total Official Google Redressal Submissions:** {len(rows)} cases on file")
    ...
    report_en.append("The operator systematically clones web templates across 18+ trade and professional niches (including Legal Services, Roofing, Auto Trade, Moving, Facade Cleaning, HVAC/Airco, Painting, Plumbing).")
    report_en.append("")
    ...
    report_en.append("4. **Automated Evasion / Whack-a-Mole Regeneration Loop:** Forensic verification proves that when individual listings are suspended or removed by Google, the syndicate immediately spawns replacement profiles at the exact same physical addresses using new Knowledge Graph tokens (`11yr...`) and sequential VoIP lines. Individual listing removal is insufficient; an account-level CID manager termination is urgently required.")
    report_en.append("")
    report_en.append("---")
    report_en.append("## 2. Official Community Escalation Threads")
    report_en.append("")
    for cp in community_posts:
        tid, curl, ctitle, cstatus = cp
        report_en.append(f"- **{ctitle}** (ID: `{tid}`): [{curl}]({curl})")
    report_en.append("")
    report_en.append("---")
    report_en.append("## 3. Master Table of Official Google Redressal Case IDs")
    report_en.append("")
    report_en.append("| Target Syndicate Name | Niche / Category | Google Redressal Case ID | Status | Verified Locations |")
    report_en.append("| :--- | :--- | :--- | :--- | :--- |")

    for r in rows:
        name, web, hq, case_id, g_status, email_v, count = r
        status_str = g_status
        if email_v:
            status_str += " (email confirmed)"
        report_en.append(f"| {name} | {web} | `{case_id}` | {status_str} | {count} |")

    report_en.append("")
    report_en.append("---")
    report_en.append("## 4. High-Priority Syndicate Clusters")
    report_en.append("")
    ...
    report_en.append("- **Personal Injury Lawyers (25 locs):** Case ID `9-8728000041558`, `3-4921000041570`")
    report_en.append("")
    ...
    report_en.append("")
    report_en.append("---")
    report_en.append("## 5. Request for Product Expert Escalation")
...
- Automated Evasion / Whack-a-Mole Loop: Forensic audits confirm that when individual listings are suspended, replacement listings are immediately regenerated at the same address under new Knowledge Graph IDs. Only account-level manager termination is effective.
...
12. Personal Injury Law (25 listings): Case IDs 9-8728000041558, 3-4921000041570
...
Official Escalation Threads on Record:
- Google Help Community Thread 469151040:
...
- Local Search Forum Thread 63398:
  https://localsearchforum.com/threads/63398/
